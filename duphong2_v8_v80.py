...
def Controller(edges, PID, current_speed, current_angle, check_err, xmax, xmin, conf, cls, right, left, straight, S, notleft, notright, lefttest, righttest):
    ...
    if not arr:
        if (check_err==1):
            ...
            angle_check = 25
            return angle_check, 50, check_err, right, left, straight, notleft, notright, lefttest, righttest
        elif (check_err==2):
            print('000000000000000000000000000000000000000000')
            angle_check = -25
            return angle_check, 50, check_err, right, left, straight, notleft, notright, lefttest, righttest
    ...
    if (right==2):
        ...
        if (float(current_speed)>69.5):
            tdelay_right = tdelay_right-0.0045
        if (float(current_speed)<69.5 and float(current_speed)>66):
            tdelay_right = tdelay_right+0.0005
        ...
        if (righttest==1 and float(current_speed)>70):
            tdelay_right = tdelay_right - 0.03
            print('                                                      ')
    ...
    if (left==2):
        ...
        if (float(current_speed)>69.5):
            tdelay_left = tdelay_left-0.0055
        if (float(current_speed)<69.5 and float(current_speed)>66):
            tdelay_left = tdelay_left+0.0005
        if (float(current_speed)<66 and float(current_speed)>59.5):
    ...
    if (straight==2):
        ...
        t3 = time.time()
        while ((time.time()-t3)<0.7):
            None
        straight=0
    ############################################################################################################################################
    """STRAIGHT"""
    if (conf > 0.85 and S>4000 and xmax<620): # can sua lai
        if (cls==2):
            straight=1
