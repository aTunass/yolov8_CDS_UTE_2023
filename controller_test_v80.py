import cv2
import time
def Controller(edges, PID, current_speed, current_angle, check_err, xmax, xmin, conf, cls, right, left, straight, S, notleft, notright, lefttest, righttest, count_check):
    angle_check=0
    rightmin=0
    leftmin=0
    cuagap=0
    ...
    """DETECT LANE"""
    line = 30
    if (float(current_speed)<58 and cls==0):
        line = 33
    arr = []
    lineRow = edges[line,:]
    ...
    if not arr:
        if (check_err==1):
            ...
            angle_check = 25
            speed = -5 + count_check*5
            count_check +=1
            return angle_check, speed, check_err, right, left, straight, notleft, notright, lefttest, righttest, count_check
        elif (check_err==2):
            print('000000000000000000000000000000000000000000')
            angle_check = -25
            speed = -5 + count_check*5
            ...
            #     angle_check = 25
            #     print('angle==========================25')
            return angle_check, 100, check_err, right, left, straight, notleft, notright, lefttest, righttest, count_check
    arrmax=max(arr)
    arrmin=min(arr)
    arrmax_turn=max(arr)
    arrmin_turn=min(arr)
    ...
    if (angle>10 and left==0 and right==0 and conf==0 and float(current_speed)>65):
        angle=25
        check_err = 1
        cuagap=1
        print('++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ +25')
    if(angle<-10 and left==0 and right==0 and conf==0 and float(current_speed)>65):
        ...
        check_err = 2
        cuagap=1
    if (arrmin>80 and arrmax>140 and float(current_speed)>60):
        angle=25
        check_err = 1
    else: check_err=0
    ...
    if (left==2):
        ...
        if (float(current_speed)>69.5):
            tdelay_left = tdelay_left-0.004
        elif (float(current_speed)<69.5 and float(current_speed)>66):
            tdelay_left = tdelay_left+0.001
    ...
    if (straight==2):
        ...
        t3 = time.time()
        while ((time.time()-t3)<0.55):
            None
        straight=0
    ############################################################################################################################################
    """STRAIGHT"""
    if (conf > 0.8 and S>4000 and xmax<620): # can sua lai
        if (cls==2):
            straight=1
    ...
    #     else: straight=0
    """TURN RIGHT"""
    if (conf > 0.85 and S>2600 and xmax<620):  # can sua lai
        if (cls==1):
            right=1
    if (right==1):  
        # line = 41
    ...
    """NO TURN RIGHT"""
    if (conf > 0.85 and S>2600 and xmax<570): # can sua lai
        if (cls==4):
            notright=1
            notleft = 0
    if (notright==1):
        if (left_detect==1):
            ...
            straight=1
    """NO TURN LEFT"""
    if (conf > 0.85 and S>2600 and xmax<635): # can sua lai
        if (cls==3):
            notleft=1
            notright=0
    if (notleft==1):
    ...
    if (check_err>0):
        speed = 30
    elif (right<2 and left<2 and straight<2):
        if (set_speed_OD>0):
            if (set_speed_OD>0):
            ...
            if (float(current_speed)<set_speed):
                ...
                if (float(current_speed)>81):
                    ...
                    if (cuagap==1):
                        ...
                        if (cls>0 and S>2000):
                            speed = -2.1*abs((error)) + 150
                        else: speed = -2.0*abs((error)) + 150
                    elif (float(current_speed)>65):
                        speed = -2*abs((error)) + 150
                    elif (float(current_speed)<60):
                        speed = 150
                    else: speed = -1.9*abs((error)) + 150
    print(arrmax, arrmin, left_detect, right_detect, xmax, xmin)
    ...
    # key = cv2.waitKey(1)
    return angle, speed, check_err, right, left, straight, notleft, notright, lefttest, righttest, count_check
